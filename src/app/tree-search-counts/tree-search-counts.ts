...
export class TreeSearchCounts implements OnInit {
    ...
    getColumnIdsSplitted() {
        return this.getColumnIds()
            .map((c) => [c + this.COLLECTION_POSTFIX, c + this.SEARCH_POSTFIX])
    ...
    private mapLRTData(dataFlat: CollectionTreeNodeStatsEntry[], stats: StatsResponse) {
        dataFlat.map((d) => {
            d.data = {
                search: this.collectHits(
                    this.lrtCombinedSKOS,
                    stats.stats[d.noderef_id]?.['search'],
                ),
                collection: this.collectHits(
                    this.lrtCombinedSKOS,
                    stats.stats[d.noderef_id]?.['material_types'],
                ),
            };
        });
        return dataFlat;
    ...
    getCount(element: CollectionTreeNodeStatsEntry, column: string) {
        return (
            element?.data?.[column.includes('_search') ? 'search' : 'collection']?.[
    ...
    showSearch(element: CollectionTreeNodeEntry, column: string) {
        ...
        window.open(
            environment.eduSharingPath +
                ...
                '&' +
                'title=' +
                encodeURIComponent('MetaQS - Suchergebnisse für "' + query + '"') +
                '&' +
                'filters=' +
    ...
    showCollectionItems(element: CollectionTreeNodeEntry, column: string) {
        ...
        window.open(
            environment.eduSharingPath +
                '/components/editorial-desk?mode=audit&' +
                'title=' +
                encodeURIComponent('MetaQS - Sammlungsinhalte für "' + element.title + '"') +
                '&' +
                'filters=' +
                encodeURIComponent(JSON.stringify(parameters)) +
                '&' +
